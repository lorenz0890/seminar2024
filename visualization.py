...
def plot_similarity(results, filename='similarity_with_accuracy_plot.png'):
    ...
    test_acc_epochs = range(1, len(results['train_acc']) + 1)  # Number of epochs for test_acc
    ...
    for i, (depth, metrics) in enumerate(results.items()):
        if isinstance(depth, int):  # Only process depths, not 'test_acc'
            ...
            inter_line, = ax1.plot(range(1, len(inter_cleaned) + 1), inter_cleaned,
                                   label=f'Depth (Inter) {depth}', linestyle='--', color=colors[i])
    ...
    ax2 = ax1.twinx()
    test_acc = results['train_acc']
    ax2.plot(test_acc_epochs, test_acc, label='Train Accuracy', color='orange', linestyle='-.', linewidth=2)
